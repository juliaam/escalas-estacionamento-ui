import { SemNome } from "../enums/semNome";
import { Sector } from "../types/Sector";
import { Sector as SectorENUM } from "@/shared/enums/sectorType";

export const sectors: Sector[] = [
  {
    id: "e7b8a6e4-8f2b-4b8d-9f2e-1c9b8a6e4f2b",
    name: "Setor 1",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6",
    name: "Setor 2",
    type: SectorENUM.enum.out,
    typeSemNome: SemNome.enum.seated,
  },
  {
    id: "b2c3d4e5-f6a7-8b9c-0d1e-2a3b4c5d6e7f",
    name: "Setor 3",
    type: SectorENUM.enum.out,
    typeSemNome: SemNome.enum.seated,
  },
  {
    id: "c3d4e5f6-a7b8-9c0d-1e2a-3b4c5d6e7f8a",
    name: "Setor 4",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "d4e5f6a7-b8c9-0d1e-2a3b-4c5d6e7f8a9b",
    name: "Setor 5",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "e5f6a7b8-c9d0-1e2a-3b4c-5d6e7f8a9b0c",
    name: "Setor 6",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "f6a7b8c9-d0e1-2a3b-4c5d-6e7f8a9b0c1d",
    name: "Setor 7",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "a7b8c9d0-e1f2-3a4b-5c6d-7e8f9a0b1c2d",
    name: "Setor 8",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "b8c9d0e1-f2a3-4b5c-6d7e-8f9a0b1c2d3e",
    name: "Setor 9",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "c9d0e1f2-a3b4-5c6d-7e8f-9a0b1c2d3e4f",
    name: "Setor 10",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
  {
    id: "d0e1f2a3-b4c5-6d7e-8f9a-0b1c2d3e4f5a",
    name: "Setor 11",
    type: SectorENUM.enum.in,
    typeSemNome: SemNome.enum.standing,
  },
];
